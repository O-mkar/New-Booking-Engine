	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new WP_Error(
				...
				sprintf(
					esc_html__( 'The customizer import file is missing! File path: %s', 'calluna-shortcodes' ),
					$import_file_path
				)
			);
		}
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new WP_Error(
				...
				esc_html__( 'The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'calluna-shortcodes' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'calluna-shortcodes' )
			);
		}
		...
		if ( isset( $data['options'] ) ) {
			...
			require CALLUNA_CORE_PATH . 'calluna-demo-importer/inc/class-ocdi-customizer-option.php';

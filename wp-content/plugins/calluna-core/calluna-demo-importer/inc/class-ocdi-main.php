...
// Include files.
require CALLUNA_CORE_PATH . 'calluna-demo-importer/inc/class-ocdi-helpers.php';
require CALLUNA_CORE_PATH . 'calluna-demo-importer/inc/class-ocdi-importer.php';
require CALLUNA_CORE_PATH . 'calluna-demo-importer/inc/class-ocdi-widget-importer.php';
require CALLUNA_CORE_PATH . 'calluna-demo-importer/inc/class-ocdi-customizer-importer.php';
require CALLUNA_CORE_PATH . 'calluna-demo-importer/inc/class-ocdi-revslider-importer.php';
require CALLUNA_CORE_PATH . 'calluna-demo-importer/inc/class-ocdi-logger.php';

/**
 * One Click Demo Import class, so we don't have to worry about namespaces.
 */
class Calluna_Core_One_Click_Demo_Import {
	 ...
	 *
	 * @return Calluna_Core_One_Click_Demo_Import the *Singleton* instance.
	 */
	...
	public function create_plugin_page() {
		$plugin_page_setup = apply_filters( 'calluna/plugin_page_setup', array(
				'parent_slug' => 'themes.php',
				'page_title'  => esc_html__( 'Calluna Demo Import' , 'calluna-shortcodes' ),
				'menu_title'  => esc_html__( 'Calluna Demo Import' , 'calluna-shortcodes' ),
				'capability'  => 'import',
				'menu_slug'   => 'calluna-one-click-demo-import',
			)
		);
	...
	<div class="ocdi wrap about-wrap">
		...
		<h2><?php esc_html_e('Calluna Demo Importer', 'calluna-shortcodes' ) ?></h2>
		...
		if ( ini_get( 'safe_mode' ) ) {
			printf(
				esc_html__( '%sWarning: your server is using %sPHP safe mode%s. This means that you might experience server timeout errors.%s', 'calluna-shortcodes' ),
		...
		<div class="ocdi__intro-notice  notice  notice-warning  is-dismissible">
			<p><?php esc_html_e( 'Before you begin, make sure all the required plugins are activated.', 'calluna-shortcodes' ); ?></p>
		</div>

		<div class="ocdi__intro-text">

			<p class="about-description">
				<?php esc_html_e( 'Importing demo data (post, pages, images, theme settings, ...) is the easiest way to setup your theme.', 'calluna-shortcodes' ); ?>
				<?php esc_html_e( 'It will allow you to quickly edit everything instead of creating content from scratch.', 'calluna-shortcodes' ); ?>
			</p>

			<hr>
			
			<div class="demoimport-importer-note">
                <strong><?php esc_html_e('Important Notes:', 'calluna-shortcodes') ?></strong>
                <ol>
                	<li><?php esc_html_e( 'No existing posts, pages, categories, images, custom post types or any other data will be deleted or modified.', 'calluna-shortcodes' ); ?></li>
                    <li><?php esc_html_e('We recommend to run the Demo Import on a clean WordPress installation.', 'calluna-shortcodes') ?></li>
                    <li><?php echo sprintf( wp_kses( __( 'To reset your installation (if the import fails) we recommend <a href="%s" target="_blank">WordPress Reset Plugin</a>.', 'calluna-shortcodes' ), array(  'a' => array( 'href' => array(), 'target' =>array() ) ) ), esc_url('https://wordpress.org/plugins/wordpress-reset/') ) ?></li>
                    <li><?php esc_html_e('The Demo Import will not import the images we have used in our live demo, due to copyright / license reasons.', 'calluna-shortcodes' ) ?></li>
                    <li><?php esc_html_e('Do not run the Demo Import multiple times, it will result in duplicated content.', 'calluna-shortcodes' ) ?></li>
                </ol>
            </div>
		...
		<?php if ( empty( $this->import_files ) ) : ?>
			...
			<div class="notice  notice-info  is-dismissible">
				<p><?php esc_html_e( 'There are no predefined import files available in this theme. Please upload the import files manually!', 'calluna-shortcodes' ); ?></p>
			</div>

			<div class="ocdi__file-upload-container">

				<h2><?php esc_html_e( 'Manual demo files upload', 'calluna-shortcodes' ); ?></h2>

				<div class="ocdi__file-upload">
					<h3><label for="content-file-upload"><?php esc_html_e( 'Choose a XML file for content import:', 'calluna-shortcodes' ); ?></label></h3>
					<input id="ocdi__content-file-upload" type="file" name="content-file-upload">
				</div>

				<div class="ocdi__file-upload">
					<h3><label for="widget-file-upload"><?php esc_html_e( 'Choose a WIE or JSON file for widget import:', 'calluna-shortcodes' ); ?></label> <span><?php esc_html_e( '(*optional)', 'calluna-shortcodes' ); ?></span></h3>
					<input id="ocdi__widget-file-upload" type="file" name="widget-file-upload">
				</div>

				<div class="ocdi__file-upload">
					<h3><label for="customizer-file-upload"><?php esc_html_e( 'Choose a DAT file for customizer import:', 'calluna-shortcodes' ); ?></label> <span><?php esc_html_e( '(*optional)', 'calluna-shortcodes' ); ?></span></h3>
		...
		<?php elseif ( 1 < count( $this->import_files ) ) : ?>
			...
			<div class="ocdi__multi-select-import">
				...
				<h2><?php esc_html_e( 'Choose which demo you want to import:', 'calluna-shortcodes' ); ?></h2>
				...
				<div class="ocdi__demo-import-preview-container">
					...
					<p><?php esc_html_e( 'Import preview:', 'calluna-shortcodes' ); ?></p>
					...
					<p class="ocdi__demo-import-preview-image-message  js-ocdi-preview-image-message"><?php
						if ( ! isset( $this->import_files[0]['import_preview_image_url'] ) ) {
							esc_html_e( 'No preview image defined for this import.', 'calluna-shortcodes' );
						}
		...
		<p class="ocdi__button-container">
			<button class="ocdi__button  button  button-hero  button-primary  js-ocdi-import-data"><?php esc_html_e( 'Import Demo Data', 'calluna-shortcodes' ); ?></button>
		</p>

		<p class="ocdi__ajax-loader  js-ocdi-ajax-loader">
			<span class="spinner"></span> <?php esc_html_e( 'Importing, please wait!', 'calluna-shortcodes' ); ?>
		</p>
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( $this->plugin_page === $hook ) {
			wp_enqueue_script( 'ocdi-main-js', CALLUNA_CORE_URL . 'calluna-demo-importer/assets/js/main.js' , array( 'jquery', 'jquery-form' ), CALLUNA_OCDI_VERSION );
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'texts'        => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'calluna-shortcodes' ),
					),
				)
			);

			wp_enqueue_style( 'ocdi-main-css', CALLUNA_CORE_URL . 'calluna-demo-importer/assets/css/main.css', array() , CALLUNA_OCDI_VERSION );
		}
	}
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'calluna-shortcodes' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					OCDI_Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'calluna-shortcodes' )
					);
				}
				...
				$log_added = OCDI_Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'calluna-shortcodes' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'calluna-shortcodes' ) );
			}
		}
		...
		if ( empty( $this->frontend_error_messages ) ) {
			$response['message'] = sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly.%5$s', 'calluna-shortcodes' ),
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'calluna-shortcodes' ),
	...
	private function import_content( $import_file_path ) {
		...
		if ( ! empty( $import_file_path ) ) {
			...
			// Add this message to log file.
			$log_added = OCDI_Helpers::append_to_file(
				$message . PHP_EOL . esc_html__( 'Max execution time after content import = ' , 'calluna-shortcodes' ) . ini_get( 'max_execution_time' ),
				$this->log_file_path,
				esc_html__( 'Importing content' , 'calluna-shortcodes' )
			);
		}
	...
	private function import_widgets( $widget_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			...
			OCDI_Helpers::log_error_and_send_ajax_response(
				...
				$this->log_file_path,
				esc_html__( 'Importing widgets', 'calluna-shortcodes' )
			);
		}
	...
	private function import_customizer( $customizer_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			...
			OCDI_Helpers::log_error_and_send_ajax_response(
				$results->get_error_message(),
				$this->log_file_path,
				esc_html__( 'Importing customizer settings', 'calluna-shortcodes' )
			);
		}

		// Add this message to log file.
		$log_added = OCDI_Helpers::append_to_file(
			esc_html__( 'Customizer settings import finished!', 'calluna-shortcodes' ),
			$this->log_file_path,
			esc_html__( 'Importing customizer settings' , 'calluna-shortcodes' )
		);
	}
	...
	private function import_revslider( $revslider_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			...
			OCDI_Helpers::log_error_and_send_ajax_response(
				$results->get_error_message(),
				$this->log_file_path,
				esc_html__( 'Importing revslider', 'calluna-shortcodes' )
			);
		}

		// Add this message to log file.
		$log_added = OCDI_Helpers::append_to_file(
			esc_html__( 'Revslider import finished!', 'calluna-shortcodes' ),
			$this->log_file_path,
			esc_html__( 'Importing revslider' , 'calluna-shortcodes' )
		);
	}
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'pt-ocdi/time_for_one_ajax_call', 25 ) ) {
			...
			$log_added = OCDI_Helpers::append_to_file(
				__( 'Completed AJAX call number: ' , 'calluna-shortcodes' ) . $this->ajax_call_number . PHP_EOL . $message,
				$this->log_file_path,
				''
			);

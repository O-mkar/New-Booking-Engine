...
<?php if ( $available_rooms > 0 && $is_available && apply_filters( 'hotelier_show_rate_add_to_cart_button', true, $checkin, $checkout, $variation ) ) : ?>
	...
	<div class="add-to-cart-wrapper add-to-cart-wrapper--rate">
			...
			<a href="#reserve-button" data-selected-text-singular="<?php echo esc_attr_x( 'room selected', 'book now button text: singular', 'hotelier' ); ?>" data-selected-text-plural="<?php echo esc_attr_x( 'rooms selected', 'book now button text: plural', 'hotelier' ); ?>" class="calluna-button btn btn-primary small button--add-to-cart"><?php esc_html_e( 'Book now', 'hotelier' ); ?></a>

...
if ( ! class_exists( 'Calluna_WPML_Config' ) ) {
	...
	class Calluna_WPML_Config {
		...
		public function __construct() {
			...
			// Add Actions
			add_action( 'admin_init', array( $this, 'calluna_register_strings' ) );
			
			add_action( 'admin_init', array( $this, 'calluna_wpml_fix_ajax_install' ) );
			
			// Add dropdown to widgets
			add_action( 'in_widget_form', array( $this, 'calluna_widget_dropdown' ), 10, 3 );

			// Update dropdown value on widget update
			add_filter( 'widget_update_callback', array( $this, 'calluna_widget_update' ), 10, 4 );
	
			// Filter widgets by language
			add_filter( 'widget_display_callback', array( $this, 'calluna_display_widget' ), 10, 3 );

			// Add Filters
			add_filter( 'upload_dir', array( $this, 'calluna_convert_base_url' ) );
		}
		 ...
		 *
		 */
		public function calluna_register_strings() {
		...
		public function calluna_wpml_fix_ajax_install(){
            global $sitepress;
		 ...
		 *
		 */
		public function calluna_convert_base_url( $param ) {
		 ...
		 */
		public function calluna_widget_dropdown( $widget, $form, $instance ) {
	
			$languages = icl_get_languages();
	
			?><p>
				<label for='wpml_language'><?php esc_html_e( 'Display on language:', 'calluna-td' ); ?> </label>
				...
				?><option <?php selected( $selected ); ?> value='all'><?php esc_attr_e( 'All Languages', 'calluna-td' ); ?></option>
		 ...
		 */
		public function calluna_widget_update( $instance, $new_instance, $old_instance, $this2 ) {
		 ...
		 */
		public function calluna_display_widget( $instance, $widget, $args ) {
